...
- 「クリックが記録されない」原因をデータベースに触らず調べられる診断画面
...
「クリックが記録されていない気がする」と感じたら、管理メニューの**楽天リンク → 診断**を開いてください。データベースに直接アクセスできない運用者でも、この一画面だけで原因の切り分けができます。

- **自分のクリックが記録されるか**: 最もよくある原因（「自分のアクセスを除外」が有効かつログイン中）を最初に、はっきり表示します
- **環境**: プラグインバージョン、DBスキーマバージョン、WordPressのタイムゾーンとサイト時間／UTCの現在時刻（この2つがずれていると「今日」のレポートに直近のクリックが出ないことがあります）、パーマリンク構造（「基本」のままだと `/go/` は一切機能しません）
- **配線の確認**: 3つのテーブルの存在と行数、現在の全プレフィックスに対するリライトルールの有無、記録を抑制しうる設定（自分のアクセス除外・重複排除秒数・保持日数・プレフィックス・対象ホスト）
- **直近の記録**: 直近10件のクリックとPV（**botも含めて表示し、明示的にマークします**。`is_bot = 1` で記録されたクリックは通常のダッシュボードには出てこないため、「記録されていない」の正体がbot判定であることがよくあります）、および本日・過去7日間のbot / 非bot別の件数
- **セルフテスト**: ボタン一つで、実際に存在するリンクに対してクリックを1件記録し、結果（記録された／されなかった、されなかった場合はその理由）を表示します。記録先は実際の `/go/` リダイレクトと同じ経路（`EventRepository`）で、作成される行はリファラが `rlt-selftest` になるため実トラフィックと混同しません

`visitor_hash`（またはそれに類する識別子）はこの画面のどこにも表示しません。
...
- 「クリックが記録されない」原因を、データベースへの直接アクセスなしで調べられる診断画面（管理メニューの**楽天リンク → 診断**）を追加
- 自分のクリックが記録されるかどうかの判定、環境情報（バージョン・タイムゾーン・パーマリンク構造）、テーブル／リライトルールの状態、直近のクリック・PV（bot含む）、セルフテスト機能を提供
- 自動更新のトグルがプラグイン一覧に表示されない不具合を修正。WordPress は更新チェック用トランジェントの `response`（更新あり）か `no_update`（最新）のいずれかに登録されているプラグインしか「更新対応」と見なさず、最新版のときは `no_update` に何も登録していなかったため、トグル自体が常に非表示になっていた
- ブログカード形式のショートコードから生成される `data-ga4-label` が、アフィリエイトの転送用URL（`hb.afl.rakuten.co.jp/...`）をそのまま反映しており、宿IDの手前がどのリンクも同じノイズになっていた不具合を修正。転送先URL（`pc` パラメータ）のホスト＋パスから生成するように変更し、出力時のフィルタでも既存のリンクに同様の変換を適用するようにした
- `data-ga4-label` を、保存済みの `label` カラムではなく**リンク自体の表示テキスト（宿名）**から生成するように変更。ブログカードのショートコードも展開後のアンカーには宿名が入っているため、GA4のレポートで読める名前になる。アンカーにテキストが無い場合は画像の `alt`、それも無い場合は従来どおり保存済みラベル（機械生成でない場合）または転送先URLから生成したラベルにフォールバックする。宿名・altから得たラベルは十分に区別可能なため宿IDは付加しない（フォールバック時のみ従来どおり付加する）
