...
- `data-ga4-label` に、アフィリエイトURLの `pc` パラメータから取得した宿IDを ` (id)` の形式で付加するように変更。記事内のボタン文言が「楽天トラベルで見る」のようにどのリンクも同じ場合でも、GA4のレポートで別の宿として区別できるようになる
- 出力時（`the_content` フィルタ）に付与する仕組みのため、**既存のリンクにも記事の再スキャンなしで次回表示時から即座に反映される**。保存済みの `label` カラム自体は変更しない
- 宿IDが取得できない場合（`pc` が無い・パスに数字のみのセグメントが無いなど）はラベルを変更しない。100文字上限を超える場合は本文側を切り詰めてIDが必ず残るようにする
